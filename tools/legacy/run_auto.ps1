...
#
# ARCHIVE BEFORE CLEARING THE LOG. The archiver copies segcap.log in beside the
# masks, because a mask set with no log cannot be explained after the fact --
# and the delete used to run first, so every archive here got the masks and lost
# their explanation. The archiver also takes segcap_frame_* now, so the explicit
# frame deletion that used to follow this line is gone: it would only ever fire
# on frames the archiver had deliberately declined to keep.
